-- ============================================================
-- SCRIPT MAESTRO AJUSTADO - Sistema de Seguro de Vehiculos
-- UASD - INF-8236-C2 - Actividad 4.2
-- Requisitos aplicados:
-- 1. Eliminar y recrear la base de datos
-- 2. Usar nombres de columnas con formato Tabla_Campo
-- 3. Insertar 20 registros en cada tabla
-- 4. Usar campo Tabla_Estatus en todas las tablas
-- Motor: SQL Server
-- ------------------------------------------------------------
-- QUE HACE ESTE SCRIPT MAESTRO
-- Este es el archivo completo del proyecto.
-- Si se ejecuta solo, deja todo listo desde cero.
--
-- RELACION CON LOS DEMAS SCRIPTS
-- Parte 1: DDL  -> crea la base y las tablas.
-- Parte 2: DML  -> inserta los 20 registros por tabla.
-- Parte 3: DQL  -> realiza las consultas del proyecto.
-- Este script maestro integra la Parte 1 y la Parte 2 en un
-- solo archivo. La Parte 3 se deja aparte en el script DQL.
--
-- CONTENIDO DEL SCRIPT
-- 1. Borra la base SeguroVehiculos si ya existe.
-- 2. Crea nuevamente la base de datos.
-- 3. Crea todas las tablas del sistema.
-- 4. Aplica claves primarias, foraneas y validaciones.
-- 5. Inserta 20 registros en cada tabla.
-- 6. Deja la base lista para consultar.
--
-- CUANDO USARLO
-- Usalo cuando quieras resolver todo con un solo archivo.
-- No necesitas ejecutar DDL, DML y DQL por separado si usas este.
--
-- RESULTADO ESPERADO
-- Al finalizar, la base SeguroVehiculos queda creada, poblada
-- y lista para pruebas o consultas academicas.
-- ============================================================
...
CREATE TABLE TipoVehiculo (
    ...
    TipoVehiculo_Tarifa_Base     DECIMAL(12,2)   NOT NULL,
    ...
    CONSTRAINT CHK_TipoVehiculo_Tarifa CHECK (TipoVehiculo_Tarifa_Base > 0),
...
CREATE TABLE Cliente (
    ...
    Cliente_Fecha_Registro        DATETIME        NOT NULL DEFAULT GETDATE(),
    ...
    CONSTRAINT CHK_Cliente_Edad CHECK (DATEDIFF(YEAR, Cliente_Fecha_Nacimiento, GETDATE()) >= 18),
...
CREATE TABLE Agente (
    ...
    Agente_Fecha_Ingreso          DATE            NOT NULL,
    Agente_Comision_Porcentaje    DECIMAL(5,2)    NOT NULL DEFAULT 5.00,
    ...
    CONSTRAINT CHK_Agente_Comision CHECK (Agente_Comision_Porcentaje BETWEEN 0 AND 100),
...
CREATE TABLE Cobertura (
    ...
    Cobertura_Prima_Base          DECIMAL(12,2)   NOT NULL,
    ...
    CONSTRAINT CHK_Cobertura_Prima CHECK (Cobertura_Prima_Base > 0),
...
CREATE TABLE Vehiculo (
    ...
    Vehiculo_Valor_Asegurado      DECIMAL(14,2)   NOT NULL,
    ...
    CONSTRAINT FK_Vehiculo_Tipo FOREIGN KEY (TipoVehiculo_ID) REFERENCES TipoVehiculo(TipoVehiculo_ID),
    CONSTRAINT CHK_Vehiculo_Anio CHECK (Vehiculo_Anio BETWEEN 1900 AND 2035),
    CONSTRAINT CHK_Vehiculo_Valor CHECK (Vehiculo_Valor_Asegurado > 0),
...
CREATE TABLE SolicitudCotizacionPoliza (
    ...
    SolicitudCotizacionPoliza_Fecha_Solicitud       DATETIME        NOT NULL,
    ...
    SolicitudCotizacionPoliza_Monto_Cotizado        DECIMAL(12,2)   NOT NULL,
    CONSTRAINT PK_SolicitudCotizacionPoliza PRIMARY KEY (SolicitudCotizacionPoliza_ID),
    CONSTRAINT FK_Solicitud_Cliente FOREIGN KEY (Cliente_ID) REFERENCES Cliente(Cliente_ID),
    CONSTRAINT FK_Solicitud_Vehiculo FOREIGN KEY (Vehiculo_ID) REFERENCES Vehiculo(Vehiculo_ID),
    CONSTRAINT FK_Solicitud_Agente FOREIGN KEY (Agente_ID) REFERENCES Agente(Agente_ID),
    CONSTRAINT FK_Solicitud_Cobertura FOREIGN KEY (Cobertura_ID) REFERENCES Cobertura(Cobertura_ID),
...
CREATE TABLE Poliza (
    ...
    SolicitudCotizacionPoliza_ID INT            NOT NULL,
    Cliente_ID                    INT             NOT NULL,
    Vehiculo_ID                   INT             NOT NULL,
    Agente_ID                     INT             NOT NULL,
    Cobertura_ID                  INT             NOT NULL,
    ...
    Poliza_Fecha_Vencimiento      DATE            NOT NULL,
    Poliza_Estatus                VARCHAR(15)     NOT NULL DEFAULT 'Activa',
    Poliza_Prima_Anual            DECIMAL(12,2)   NOT NULL,
    ...
    CONSTRAINT FK_Poliza_Solicitud FOREIGN KEY (SolicitudCotizacionPoliza_ID) REFERENCES SolicitudCotizacionPoliza(SolicitudCotizacionPoliza_ID),
    CONSTRAINT FK_Poliza_Cliente FOREIGN KEY (Cliente_ID) REFERENCES Cliente(Cliente_ID),
    CONSTRAINT FK_Poliza_Vehiculo FOREIGN KEY (Vehiculo_ID) REFERENCES Vehiculo(Vehiculo_ID),
    CONSTRAINT FK_Poliza_Agente FOREIGN KEY (Agente_ID) REFERENCES Agente(Agente_ID),
    CONSTRAINT FK_Poliza_Cobertura FOREIGN KEY (Cobertura_ID) REFERENCES Cobertura(Cobertura_ID),
    CONSTRAINT CHK_Poliza_Estatus CHECK (Poliza_Estatus IN ('Activa', 'Vencida', 'Cancelada', 'Inactiva')),
    CONSTRAINT CHK_Poliza_Prima CHECK (Poliza_Prima_Anual > 0),
    CONSTRAINT CHK_Poliza_Fechas CHECK (Poliza_Fecha_Vencimiento > Poliza_Fecha_Emision)
);
GO
...
CREATE TABLE FacturaPoliza (
    ...
    FacturaPoliza_Fecha_Emision  DATETIME        NOT NULL,
    FacturaPoliza_Monto_Total    DECIMAL(12,2)   NOT NULL,
    ...
    CONSTRAINT FK_FacturaPoliza_Poliza FOREIGN KEY (Poliza_ID) REFERENCES Poliza(Poliza_ID),
    CONSTRAINT CHK_FacturaPoliza_Estatus CHECK (FacturaPoliza_Estatus IN ('Registrada', 'Entregada', 'Pagada', 'Anulada')),
    CONSTRAINT CHK_FacturaPoliza_Monto CHECK (FacturaPoliza_Monto_Total > 0)
);
GO

CREATE TABLE Pago (
    Pago_ID                       INT             IDENTITY(1,1) NOT NULL,
    FacturaPoliza_ID             INT             NOT NULL,
    Cliente_ID                    INT             NOT NULL,
    ...
    CONSTRAINT FK_Pago_Factura FOREIGN KEY (FacturaPoliza_ID) REFERENCES FacturaPoliza(FacturaPoliza_ID),
...
CREATE TABLE Siniestro (
    ...
    Siniestro_Fecha_Ocurrencia    DATE            NOT NULL,
    Siniestro_Fecha_Reporte       DATETIME        NOT NULL,
    Siniestro_Tipo                VARCHAR(20)     NOT NULL,
    Siniestro_Descripcion         VARCHAR(500)    NOT NULL,
    Siniestro_Lugar_Ocurrencia    VARCHAR(200)    NOT NULL,
    Siniestro_Monto_Estimado_Dano DECIMAL(12,2)   NOT NULL DEFAULT 0.00,
    ...
    CONSTRAINT FK_Siniestro_Poliza FOREIGN KEY (Poliza_ID) REFERENCES Poliza(Poliza_ID),
    CONSTRAINT FK_Siniestro_Vehiculo FOREIGN KEY (Vehiculo_ID) REFERENCES Vehiculo(Vehiculo_ID),
    ...
    CONSTRAINT CHK_Siniestro_Monto CHECK (Siniestro_Monto_Estimado_Dano >= 0)
);
GO
...
CREATE TABLE EvaluacionSiniestro (
    ...
    EvaluacionSiniestro_Descripcion_Danos        VARCHAR(600)    NOT NULL,
    EvaluacionSiniestro_Monto_Aprobado           DECIMAL(12,2)   NOT NULL DEFAULT 0.00,
    ...
    CONSTRAINT FK_EvaluacionSiniestro_Siniestro FOREIGN KEY (Siniestro_ID) REFERENCES Siniestro(Siniestro_ID),
    CONSTRAINT FK_EvaluacionSiniestro_Taller FOREIGN KEY (Taller_ID) REFERENCES Taller(Taller_ID),
    CONSTRAINT CHK_EvaluacionSiniestro_Estatus CHECK (EvaluacionSiniestro_Estatus IN ('Pendiente', 'Completada', 'Rechazada', 'Anulada')),
    CONSTRAINT CHK_EvaluacionSiniestro_Monto CHECK (EvaluacionSiniestro_Monto_Aprobado >= 0)
);
GO

INSERT INTO TipoVehiculo (
    TipoVehiculo_Descripcion,
    TipoVehiculo_Tarifa_Base
) VALUES
...
INSERT INTO Agente (
    ...
    Agente_Telefono,
    Agente_Email,
    Agente_Fecha_Ingreso,
    Agente_Comision_Porcentaje
) VALUES
...
INSERT INTO SolicitudCotizacionPoliza (
    ...
    Agente_ID,
    Cobertura_ID,
    SolicitudCotizacionPoliza_Fecha_Solicitud,
    SolicitudCotizacionPoliza_Estatus,
    SolicitudCotizacionPoliza_Observaciones,
    SolicitudCotizacionPoliza_Monto_Cotizado
) VALUES
...
INSERT INTO FacturaPoliza (
    ...
    FacturaPoliza_Fecha_Emision,
    FacturaPoliza_Monto_Total,
...
INSERT INTO Siniestro (
    ...
    Siniestro_Fecha_Reporte,
    Siniestro_Tipo,
    Siniestro_Descripcion,
    Siniestro_Lugar_Ocurrencia,
    Siniestro_Monto_Estimado_Dano,
    Siniestro_Estatus
) VALUES
...
INSERT INTO EvaluacionSiniestro (
    ...
    EvaluacionSiniestro_Descripcion_Danos,
    EvaluacionSiniestro_Monto_Aprobado,
...
PRINT '============================================================';
PRINT 'Base de datos SeguroVehiculos recreada correctamente.';
PRINT 'Formato de columnas aplicado: Tabla_Campo.';
PRINT 'Campo Tabla_Estatus aplicado en todas las tablas.';
PRINT 'Cada tabla contiene 20 registros insertados.';
PRINT '============================================================';
GO

SELECT 'TipoVehiculo' AS Tabla, COUNT(*) AS Cantidad FROM TipoVehiculo
UNION ALL
